...
const RippleGrid: React.FC<Props> = ({
  ...
  useEffect(() => {
    // 1. Capture the ref value here.
    const container = containerRef.current;
    if (!container) return;

    // ... (Your existing OGL setup code)
...
void main() {
    ...
    // This cleanup function will now correctly use the `container` variable.
    return () => {
  ...
  // The second useEffect from your previous example is no longer needed
  // because we've combined the logic into a single, comprehensive effect.
  
  return (
    <div
      ref={containerRef}
      className="w-full min-h-screen relative overflow-hidden [&_canvas]:block bg-black"
    />
  );
};
